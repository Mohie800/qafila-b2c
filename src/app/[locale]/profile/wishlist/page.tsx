    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        {items.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-gray-border dark:border-gray-700 px-3 py-2 text-xs font-medium text-gray-text transition-colors hover:border-discount hover:text-discount"
          >
            <Trash2 size={14} />
            {t("clearAll")}
          </button>
      ...
      {clearConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="mx-4 w-full max-w-sm rounded-xl bg-white dark:bg-dark p-6 shadow-xl">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4">
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="group relative rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark"
                >
                  {/* Remove button */}
                  <button
                  ...
                  <Link href={`/products/${item.productSlug}`}>
                    <div className="relative aspect-[3/4] overflow-hidden rounded-t-lg bg-gray-100 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700">
                      {item.productImage ? (
                        <Image
                          src={
                  ...
                  <div className="space-y-1.5 p-3">
                    <Link href={`/products/${item.productSlug}`}>
                      <h3 className="text-sm font-bold uppercase tracking-wide text-dark dark:text-gray-100 line-clamp-2">
                        {title}
                      </h3>
                    </Link>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-dark dark:text-gray-100" dir="ltr">
                        <SarIcon />{" "}
                        {Number(
